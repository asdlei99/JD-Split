...
class Consign:
    def __init__(self):
        cl = class_logger
        cl.init()
        self.logger = cl.getLogger('Class_Consign')

    def add(self, addr_id, Cookie):
        headers = {
        ...
        response = requests.post(url= url, headers= headers, data= post).text
    ...
    def getAddressList(self,Cookie,skuid = '4957824'):
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.113 Safari/537.36",
            "Content-Type": "application/x-www-form-urlencoded",
            "Cookie": Cookie
        ...
        Add_Arr = json.loads(requests.get(url= url, headers= headers).text)
        return Add_Arr

    def setOnekey(self, Cookie, addr_id):
        ...
        if str(addr_id) in requests.post(url= url, headers= headers, data= post).text:
