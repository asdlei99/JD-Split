...
class Presell:
    def __init__(self):
        cl = class_logger
        cl.init()
        self.logger = cl.getLogger('Class_Presell')

    def getMyPresell(self,Cookie):
    ...
    def goPresellInfo(self,Cookie,skuid):
        ...
        return json.loads(re.findall(r"fetchJSON\((.*?)\)",requests.get(url= url, headers= headers).text)[0])

    def goPresell(self,Cookie,skuid,url):
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.113 Safari/537.36",
            "Content-Type": "application/x-www-form-urlencoded",
